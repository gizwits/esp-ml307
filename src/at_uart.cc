...
// AtUart 构造函数实现
AtUart::AtUart(gpio_num_t tx_pin, gpio_num_t rx_pin, gpio_num_t dtr_pin, uart_port_t uart_num, gpio_num_t ri_pin)
...
bool AtUart::ParseResponse() {
    ...
    // Use unique_lock so we can unlock early before calling HandleUrc
    std::unique_lock<std::mutex> lock(rx_buffer_mutex_);
    ...
    if (rx_buffer_[0] == '+') {
        ...
        // 当等待命令响应时，将 + 开头的响应行追加到 response_（供 GetApn 等需要读取多行响应的场景使用）
        if (wait_for_response_) {
            std::lock_guard<std::mutex> response_lock(mutex_);
            response_.append(rx_buffer_.substr(0, end_pos));
            response_.append("\r\n");
        }
        ...
        return true;
    } else if ((uint8_t)rx_buffer_[0] == 0xE0) { // 4G wake up MCU, just ignore
        rx_buffer_.erase(0, end_pos + 2);
        return true;
    } else {
        {
    ...
    // Release rx_buffer_ lock before calling HandleUrc
    lock.unlock();
